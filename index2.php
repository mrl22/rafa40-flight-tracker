...
if ($_GET['get_markers']) {
    ...
    if ($latest->count()) {
        foreach ($latest as $row) {
            if ($prev === true) $user_gps = json_decode($row->user_prevgps);
            ...
            if ($user_gps->user_id)
                $markers[$row->user_id] = array(
                    ...
                    'ago' => \Carbon\Carbon::parse($row->user_modified)
                        ->diffForHumans(\Carbon\Carbon::now(),  ['syntax' => \Carbon\CarbonInterface::DIFF_RELATIVE_TO_NOW]),
                    'history' => $hist
                );
        }
    }
...
if ($_GET['get_sliders']) {
    $sliders = $db::table('tracking')
//        ->where('track_speed', '>', 0)
        ->groupBy('sliderDate')
        ->orderBy('track_timestamp')
        ->select(array(
            $db::connection()->raw('DATE_FORMAT(`track_timestamp`, \'%d/%m/%Y %H:%i\') as sliderDate'),
            $db::connection()->raw('unix_timestamp(DATE_FORMAT(`track_timestamp`, \'%Y/%m/%d %H:%i\')) as unixDate')
        ))
        ->get();
    echo json_encode($sliders, true);
    exit;
}
if ($_GET['get_moment']) {
    ...
    if ($latest->count()) {
        ...
        foreach ($latest as $row) {
            ...
            $user_gps = $db::table('tracking')
                ->where('user_id', $row->user_id)
                ->where($db::connection()->raw('unix_timestamp(track_timestamp)'), '<=', (int)$_GET['get_moment'])
                ->take(1)
                ->orderBy('track_timestamp', 'desc')
                ->first();
            ...
            if ($user_gps->user_id)
                $markers[$row->user_id] = array(
                    ...
                    'bearing' => $user_gps->track_bearing,
                    'speed' => $user_gps->track_speed,
                    'ago' => \Carbon\Carbon::parse($user_gps->track_timestamp)
                        ->diffForHumans(\Carbon\Carbon::now(),  ['syntax' => \Carbon\CarbonInterface::DIFF_RELATIVE_TO_NOW])
                );
        }
    }
...
<script>
    ...
    var easing = 60 * 1000;
    var refresh_count = 0;
    var disable_follow = false, disable_refresh = false;

    function update_marker_positions() {
        // Clear Trail
        for (var i in dots) {
            dots[i].setMap(null);
        }
    ...
    function refresh_markers() {
        if (disable_refresh !== false) return;
        jQuery.get({
            url: '',
            cache: false,
            data: {'get_markers': refresh_count !== 1},
    ...
    function follow_markers() {
        if (disable_follow !== false) return;
    ...
    function update_slider() {
        jQuery.get({
            url: '',
            cache: false,
            data: {'get_sliders': true},
            success: function (data) {
                slider_data = data;
                jQuery('.slider').slider("option", "max", Object.keys(slider_data).length-1);
                jQuery('.slider').slider("value", Object.keys(slider_data).length-1 );
                jQuery('.slider-value').text(slider_data[Object.keys(slider_data).length-1].sliderDate);
            },
            dataType: 'json'
        });
    }
    ...
    function map_initialize() {
        ...
        // map.addListener('zoom_changed', disable_check );
        map.addListener('drag', disable_check);
        ...
        $(".slider").slider({
            ...
            slide : function(event, ui) {
                $('.slider-value').text(slider_data[ui.value].sliderDate);
                $.get({
                    url: '',
                    cache: true,
                    data: {'get_moment': slider_data[ui.value].unixDate},
                    success: function (data) {
                        marker_data = data;
        ...
        setInterval('update_slider()', refresh * 1000);
        update_slider();
    }
